...
`launch.sh --local` delegates to it. The wrapper fixes Node 22.22.3 and EAS CLI 21.0.0, runs one
...
only its exit code; detailed diagnosis must use the authenticated EAS surface after the signing
material described below has been rotated. The versioned redactor remains defense-in-depth and is
