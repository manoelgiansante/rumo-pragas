...
function createFixture(
  ...
  const fixtureScripts = new Set(scripts);
  const includesStoreValidator = fixtureScripts.has('validate-store-assets.mjs');
  const includesSubmissionStatus = fixtureScripts.has('store-submission-status.mjs');
  if (includesStoreValidator) fixtureScripts.add('validate-data-safety.mjs');
  ...
  const appJson = JSON.parse(readFileSync(join(sourceAppRoot, 'app.json'), 'utf8'));
  appJson.expo.version = initialVersion;
  writeFileSync(join(root, 'app.json'), JSON.stringify(appJson));
  ...
  if (includesStoreValidator) {
    mkdirSync(join(root, 'app'), { recursive: true });
    copyFileSync(join(sourceAppRoot, 'app', 'privacy.tsx'), join(root, 'app', 'privacy.tsx'));
    copyFileSync(
      join(sourceAppRoot, 'store-assets', 'android', 'pragas-datasafety-filled.csv'),
      join(root, 'store-assets', 'android', 'pragas-datasafety-filled.csv'),
    );
      ...
      `${JSON.stringify(
        {
          schemaVersion: 1,
          status: 'verified',
          scope: 'full-shared-agrorumo-account-deletion',
          accountDeletionUrl: 'https://pragas.agrorumo.com/delete-account',
          verifiedAt: '2026-07-15T12:00:00.000Z',
          reviewer: 'Independent Privacy Reviewer',
          legalDecisionReference: 'privacy-review-2026-07-15',
          integrationEvidenceSha256: 'a'.repeat(64),
        },
        null,
        2,
      )}\n`,
  ...
  const initialCommitFiles = ['expo-app/package.json', 'expo-app/app.json'];
  if (includesSubmissionStatus) {
    initialCommitFiles.push('expo-app/store-assets/ACCOUNT_DELETION_RESOLUTION.json');
  }
  const gitAdd = spawnSync('git', ['add', ...initialCommitFiles], {
    cwd: repository,
    encoding: 'utf8',
  });
...
test('candidate artifact identifiers cannot be placeholder UUIDs or local hashes', (t) => {
  const root = createFixture(t);
  populateValidAssets(root);
  writeManifest(root, (manifest) => {
  ...
  assert.match(result.stderr, /sha256 deve ser.*artefato local/);
});

test('selected local artifact hash must match the reviewed manifest', (t) => {
  const root = createFixture(t);
  populateValidAssets(root);
...
test('store asset validator rejects remote build IDs before inventory validation', (t) => {
  const root = createFixture(t);
  populateValidAssets(root);
  ...
  assert.match(result.stderr, /--build-id não é aceito; informe um artefato local/);
});

test('CI inventory mode rejects a manifest backed only by a remote build UUID', (t) => {
  const root = createFixture(t);
  populateValidAssets(root);
  writeManifest(root, (manifest) => {
    manifest.platforms.ios.candidateArtifact.sha256 = null;
  });
  const result = runNode(root, 'validate-store-assets.mjs', ['ios']);
  assert.equal(result.status, 3, outputOf(result));
  assert.match(result.stderr, /candidateArtifact\.sha256 deve ser.*artefato local/);
});
...
test('global submission status blocks account deletion even when every asset is valid', (t) => {
  ...
  writeFileSync(resolutionPath, `${resolution.trimEnd()}\n`);
  result = runNode(root, 'store-submission-status.mjs');
  assert.equal(result.status, 0, outputOf(result));
  assert.match(result.stdout, /STORE_SUBMISSION_STATUS=ASSETS_READY/);

  writeFileSync(resolutionPath, resolution.replace('verified', 'tampered'));
  result = runNode(root, 'store-submission-status.mjs');
  assert.equal(result.status, 3, outputOf(result));
  assert.match(result.stderr, /idêntica ao commit candidato/u);
});
