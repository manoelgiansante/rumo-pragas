...
if (!['ios', 'android'].includes(platform)) {
  console.error(
    'Uso: node scripts/validate-store-assets.mjs <ios|android> [--build-id UUID | --artifact CAMINHO]',
  );
  process.exit(2);
}

const submissionContext = { buildId: null, artifact: null };
for (let index = 3; index < process.argv.length; index += 1) {
  const argument = process.argv[index];
  if (argument === '--build-id' && index + 1 < process.argv.length) {
    submissionContext.buildId = process.argv[index + 1];
    index += 1;
  } else if (argument === '--artifact' && index + 1 < process.argv.length) {
...
if (submissionContext.buildId !== null && submissionContext.artifact !== null) {
  console.error('ERRO: informe no máximo um --build-id ou --artifact ao validar ativos.');
  process.exit(2);
}
if (submissionContext.buildId !== null && !uuidPattern.test(submissionContext.buildId)) {
  console.error('ERRO: --build-id deve ser um UUID canônico.');
  process.exit(2);
}
...
function validateManifest(requestedPlatform, sets) {
  ...
  if (
    ...
    if (
      ...
      manifestError(`${artifactContext}.sha256 deve ser null ou SHA-256 minúsculo não provisório.`);
    }
    if (candidateArtifact.buildId === null && candidateArtifact.sha256 === null) {
      manifestError(`${artifactContext} deve declarar buildId e/ou sha256 imutável.`);
    }

    if (
      submissionContext.buildId !== null &&
      candidateArtifact.buildId !== submissionContext.buildId
    ) {
      manifestError(`${artifactContext}.buildId não corresponde ao artefato selecionado.`);
    }
