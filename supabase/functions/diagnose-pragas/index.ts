...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // invalid_image contract below WITHOUT spending an Agrio/Claude call (and
    // without taking the provider lease — no provider side effects happen). Any
    ...
    const plantGate = await runPlantGate({
      ...
      onError: (gateError) =>
        captureException(gateError, {
          ...
          tags: { fn: "diagnose-pragas", step: "plant_gate" },
          extra: { requestId },
        }),
    });
    ...
    if (plantGate === "blocked") {
      ...
      // provider (paid) is never called on this path and the idempotency
      // provider lease is intentionally NOT taken (no provider side effects).
      logJson(
        "diagnose",
        requestId,
        "INFO",
        "Plant gate blocked non-plant image — provider skipped",
      );
      diagnosisData = {
