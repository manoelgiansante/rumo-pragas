...
export async function callAgrioDiagnose(opts: {
  ...
  try {
    ...
    throw new Error(`Agrio non-JSON ${res.status}: ${text.slice(0, 200)}`);
  }
...
export const AGRIO_LABEL_MAP: Record<string, Record<string, KbHint>> = {
  ...
  Grape: {
    ...
    // gap on the dedicated slug). Generic label → PT translation ONLY: a
    // generic fungal call has no single scientific name, so none is invented —
    // the client keeps degrading gracefully without a catalog match.
    fungaldisease: {
...
export function adaptAgrio(
  ...
  const cropConfidence =
    typeof raw.cropConfidence === "string"
  ...
  // Coverage telemetry: a top prediction with neither a scientific name nor a
  // bridge hint cannot resolve the PT catalog → log the exact Agrio label so we
  // extend AGRIO_LABEL_MAP from real traffic (never blocks the response).
  if (!top.scientificName && !topHint) {
    captureException(new Error(`Agrio label unmapped: ${cropEn || "?"}/${top.id ?? top.commonName}`), {
      level: "info",
      tags: {
        fn: "diagnose",
        step: "agrio_label_unmapped",
        crop: cropEn || "unknown",
        label: String(top.id ?? top.commonName ?? "unknown"),
      },
    }).catch(() => {});
  }
