...
--
-- This migration is additive and data-preserving at migration time: it does
-- not UPDATE or DELETE existing rows. Runtime mutation is exposed only through
-- identity-derived or service-role-only functions with explicit grants.
...
BEGIN
  IF NEW.verdict IS NULL THEN
    ...
    END;
  END IF;
  IF NEW.feedback IS NULL OR NEW.feedback = 'unsure' THEN
    NEW.feedback := CASE NEW.verdict
...
BEGIN
  ...
  IF to_regclass('public.pragas_notification_queue') IS NOT NULL THEN
    ...
    END LOOP;
    SELECT coalesce(array_agg(owned.token_value), ARRAY[]::text[])
      INTO v_owned_tokens
      FROM unnest(v_owned_tokens) AS owned(token_value)
     WHERE NOT EXISTS (
       SELECT 1 FROM public.pragas_push_tokens AS current_owner
        WHERE current_owner.user_id <> p_user_id
          ...
          AND (current_owner.token = owned.token_value
            OR current_owner.expo_token = owned.token_value)
     );
    DELETE FROM public.pragas_notification_queue
     WHERE token = ANY (v_owned_tokens);
    GET DIAGNOSTICS v_count = ROW_COUNT;
